...
from arteryseg.modeling.roi_heads.fast_rcnn import FastRCNNFocaltLossOutputLayers
...
class StandardROIHeadsPseudoLab(StandardROIHeads):
    ...
    def forward(
        ...
        if self.training and compute_loss:  # apply if training loss
            ...
            if len(storage._vis_data) and branch=='debug':
                img = [m[1] for m in storage._vis_data if m[0]=='org_image'][0]
                img = img.permute((1,2,0)).numpy()
                pbox = proposals[0].proposal_boxes.tensor.cpu().numpy()
                label = proposals[0].gt_classes.cpu().numpy()
                posb = pbox[label==0]
                negb = pbox[label==1]
                img1 = vis_box(img, negb, (255,0,0))
                img1 = img1[np.newaxis,:,:].repeat(3,0)
                storage.put_image('before_roi_neg', img1)
                img2 = vis_box(img, posb, (255,0,0))
                img2 = img2[np.newaxis,:,:].repeat(3,0)
                storage.put_image('before_roi_pos', img2)
    ...
    def _forward_box(
        ...
        if (
            ...
            # List[Instances] topk most confidence detections
            # _ = kept_indices
            
            # List[Instances], (scores, proposal_deltas)
            return pred_instances, predictions
    ...
    def _forward_mask(self, features: Dict[str, torch.Tensor], instances: List[Instances]):
        ...
        if self.mask_pooler is not None:
            ...
            for box in boxes:
                box.tensor = box.tensor + torch.tensor([[-5, -5, 5, 5]], device=box.tensor.device)
            #print(boxes)
